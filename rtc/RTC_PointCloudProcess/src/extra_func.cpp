...
GetDirection(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_in, float x,float y,float z,float radius, float& angle,float tolerance)
{
  ...
  int interval_num;
  if(radius > 0.134) // 0.14
  {
    std::cout << "the plate is too big to be caught!" << std::endl;
    angle = pi;
    return 0;
  }
  else if(radius > 0.124)  // 0.13
  {
    interval[0] = radius+0.01;
    interval[1] = 0.14;
    interval_num = 1;
  }
  else if(radius > 0.114)  // 0.12
  {
    interval[0] = radius+0.01;
    interval[1] = 0.13;
    interval[2] = 0.14;
  ...
  for(int i = 0; i < interval_num; i++)
  {
    ...
    //ShowCloud(cloud);
    float limit = z + h_tool - delta - 0.02;
    if(limit > z + 0.002)
    {
      PrePassThrough(cloud,cloud,limit,z+h_tool+1,"z");            // h_tool is in need
      std::cout << "here shows the height for the filter : " << z+h_tool-delta-0.02 << std::endl;
    }
    else
    {
      PrePassThrough(cloud,cloud,z + 0.002,z+h_tool+1,"z");            // h_tool is in need
      std::cout << "here shows the height for the filter : " << z+0.002 << std::endl;
    }
  ...
  if(angles_poss[0].theta_range > pi + angle_inneed + pi/36)   // check if the biggest interval has already covered the position to catch.
  {
...
checkFood(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_in,PlateInformation& infor)
{
  ...
  if(cloud->points.size() > 100)
  {
    ...
    //ShowCloud(cloud);
    WriteCloud(cloud,"food_on_plate.pcd");
  }
